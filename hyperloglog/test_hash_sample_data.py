...
HASH_DIR = SCRIPT_DIR / "data" / "01-hash"
JAR_PATH = SCRIPT_DIR / "app" / "build" / "libs" / "app.jar"


def run_hash_sample(input_path: Path) -> str:
    if not JAR_PATH.exists():
        raise FileNotFoundError(f"Jar not found at {JAR_PATH}")

    proc = subprocess.Popen(
        ["java", "-jar", str(JAR_PATH), "hash-sample"],
...
def collect_inputs(args: Iterable[str]) -> List[Path]:
    if not args:
        return sorted(HASH_DIR.glob("**/*.in"))

    inputs: List[Path] = []
    for raw in args:
        candidate = Path(raw)
        if not candidate.is_absolute():
            candidate = HASH_DIR / candidate
...
def main(argv: List[str]) -> int:
    ...
    if not inputs:
        print("No hash inputs found.", file=sys.stderr)
        return 1
    ...
    for input_path in inputs:
        ...
        actual_lines = run_hash_sample(input_path).splitlines()

        rel = input_path.relative_to(HASH_DIR)
    ...
    if failures:
        print(f"{failures} failing hash sample test(s).", file=sys.stderr)
        return 1

    print(f"All {len(inputs)} hash sample test(s) passed.")
    return 0
