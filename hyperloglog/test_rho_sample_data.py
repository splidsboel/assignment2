...
def collect_inputs(args: Iterable[str]) -> List[Path]:
    if not args:
        return sorted(RHO_DIR.glob("**/*.in"))

    inputs: List[Path] = []
    for raw in args:
        candidate = Path(raw)
        if not candidate.is_absolute():
            candidate = RHO_DIR / candidate
...
def main(argv: List[str]) -> int:
    ...
    if not inputs:
        print("No rho inputs found.", file=sys.stderr)
        return 1
    ...
    for input_path in inputs:
        ...
        actual_lines = run_rho_sample(input_path).splitlines()

        rel = input_path.relative_to(RHO_DIR)
    ...
    if failures:
        print(f"{failures} failing rho sample test(s).", file=sys.stderr)
        return 1

    print(f"All {len(inputs)} rho sample test(s) passed.")
    return 0
